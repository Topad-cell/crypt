...
def detect_head_and_shoulders(df, order=5, threshold=0.02):
    ...
    for i in range(len(indices)-2):
        l, h, r = highs[indices[i]], highs[indices[i+1]], highs[indices[i+2]]
        if h > l and h > r and abs(l - r)/h < threshold:
            patterns.append({
...
def detect_inverse_head_and_shoulders(df, order=5, threshold=0.02):
    ...
    patterns = []
    for i in range(len(indices)-2):
        l, h, r = lows[indices[i]], lows[indices[i+1]], lows[indices[i+2]]
        if h < l and h < r and abs(l - r)/h < threshold:
            patterns.append({
...
def detect_symmetrical_triangle(df, order=5, threshold=0.02):
    ...
    for i in range(min(len(max_idx), len(min_idx))-1):
        ...
        if len(segment_high) > 1 and len(segment_low) > 1:
            if (segment_high[0] > segment_high[-1]) and (segment_low[0] < segment_low[-1]):
                patterns.append({
                    'type': 'SymmetricalTriangle',
                    'indices': [max_idx[i], max_idx[i+1], min_idx[i], min_idx[i+1]],
                    'direction': 'neutral'
                })
    return patterns
...
def detect_channel(df, order=5, min_length=10, threshold=0.01):
    ...
    if len(max_idx) > 1 and len(min_idx) > 1:
        ...
        top_slope = (highs[max_idx[-1]] - highs[max_idx[0]]) / (max_idx[-1] - max_idx[0])
        bot_slope = (lows[min_idx[-1]] - lows[min_idx[0]]) / (min_idx[-1] - min_idx[0])
        if abs(top_slope - bot_slope) / (abs(top_slope)+1e-8) < threshold:
            if abs(max_idx[-1] - max_idx[0]) > min_length and abs(min_idx[-1] - min_idx[0]) > min_length:
                patterns.append({
                    'type': 'Channel',
